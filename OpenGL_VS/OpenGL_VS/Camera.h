...
public:
  ...
  Camera(glm::vec3 position, glm::vec3 up, glm::vec3 la, float fieldOfViewInY, float n, float f):eyePosition(position), upVector(up), lookAt(la), fovy(fieldOfViewInY), _near(n), _far(f){
  ...
  float halfHeightNear( ){
    ...
    float hh = tan(fovy_rads / 2.0) * _near;
    return hh;
  }
  ...
  float halfHeightFar( ){
    float fovy_rads = fovy * (3.14159265358979323846 / 180.0);
	//float fovy_rads = fovy * (M_PI / 180.0);
    float hh = tan(fovy_rads / 2.0) * _far;
    return hh;
  }
  ...
  void drawViewFrustum(float windowAspectRatio){
    ...
    glm::vec3 nearCenter = c + gaze( ) * _near;
    glm::vec3 farCenter = c + gaze( ) * _far;
  ...
  void perspectiveMatrix(glm::mat4& m, float windowAspectRatio){
    m = glm::perspective(deg2rad(fovy), windowAspectRatio, _near, _far);
  }
